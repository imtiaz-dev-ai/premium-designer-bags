...
import { getCart, removeFromCart, updateQty, getCartTotal, type CartItem } from "@/lib/cart-store";
...
export default function CartPage() {
  ...
  function remove(id: string, size: string) { removeFromCart(id, size); setItems(getCart()); }
      ...
      <main className="mx-auto max-w-7xl px-4 py-10">
        {items.length === 0 ? (
          <div className="py-32 text-center">
            ...
            <button onClick={() => navigate("/")} className="mt-6 inline-flex items-center gap-2 rounded-xl px-8 py-3 text-sm font-semibold text-cream" style={{ background: "var(--gradient-luxe)" }}>
              Shop Now
            </button>
          </div>
        ) : (
          <div className="grid gap-10 lg:grid-cols-[1fr_360px]">
                  ...
                  <div className="flex flex-1 flex-col justify-between">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3 rounded-lg border border-border px-3 py-1.5">
                        <button onClick={() => changeQty(item.id, item.size, item.quantity - 1)} className="text-muted-foreground hover:text-ink"><Minus className="h-3.5 w-3.5" /></button>
                        <span className="w-5 text-center text-sm font-semibold">{item.quantity}</span>
                        <button onClick={() => changeQty(item.id, item.size, item.quantity + 1)} className="text-muted-foreground hover:text-ink"><Plus className="h-3.5 w-3.5" /></button>
                      </div>
            ...
            <div className="h-fit rounded-3xl border border-border bg-card p-6 shadow-sm">
              ...
              <div className="space-y-3 text-sm">
                ...
                <div className="flex justify-between"><span className="text-muted-foreground">Shipping</span><span className="font-medium">${shipping.toFixed(2)}</span></div>
              ...
              <button onClick={() => navigate("/checkout")} className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl py-4 text-sm font-semibold uppercase tracking-widest text-cream transition hover:opacity-90" style={{ background: "var(--gradient-luxe)" }}>
                Proceed to Checkout <ArrowRight className="h-4 w-4" />
              </button>
              <a href={WHATSAPP_LINK} className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl border-2 border-burgundy py-3 text-sm font-semibold text-burgundy transition hover:bg-burgundy hover:text-cream">
