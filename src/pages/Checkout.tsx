    ...
    <div className="min-h-screen bg-background" style={{ fontFamily: "var(--font-sans)" }}>
      <header className="sticky top-0 z-50 border-b border-border bg-background/90 backdrop-blur-md">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4">
          <button onClick={() => step === "payment" ? setStep("details") : navigate("/cart")} className="flex items-center gap-2 text-sm font-medium hover:text-burgundy">
        ...
        <div className="grid gap-10 lg:grid-cols-[1fr_340px]">
            ...
            {step === "details" ? (
              <div className="rounded-3xl border border-border bg-card p-8">
                ...
                <div className="grid gap-5 sm:grid-cols-2">
                  ...
                  <div className="sm:col-span-2"><Field label="Full Address *" value={form.address} onChange={(v) => set("address", v)} error={errors.address} placeholder="123 Main Street, Apt 4B" /></div>
                  <Field label="City *" value={form.city} onChange={(v) => set("city", v)} error={errors.city} placeholder="New York" />
                </div>
                <button onClick={() => { if (validateDetails()) setStep("payment"); }} className="mt-8 flex w-full items-center justify-center gap-2 rounded-xl py-4 text-sm font-semibold uppercase tracking-widest text-cream transition hover:opacity-90" style={{ background: "var(--gradient-luxe)" }}>
                  Continue to Payment →
                </button>
              </div>
            ) : (
              <div className="rounded-3xl border border-border bg-card p-8">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  {PAYMENT_OPTIONS.map((p) => (
                    <button key={p} onClick={() => setPayment(p)} className={`rounded-2xl border-2 px-5 py-4 text-left transition ${payment === p ? "border-burgundy bg-burgundy/5" : "border-border hover:border-gold"}`}>
                ...
                <button onClick={placeOrder} disabled={loading} className="mt-6 flex w-full items-center justify-center gap-2 rounded-xl py-4 text-sm font-semibold uppercase tracking-widest text-cream transition hover:opacity-90 disabled:opacity-60" style={{ background: "var(--gradient-luxe)" }}>
          ...
          <div className="h-fit rounded-3xl border border-border bg-card p-6">
              ...
              {items.map((item) => (
                <div key={`${item.id}-${item.size}`} className="flex gap-3">
                  <div className="relative">
                    ...
                    <span className="absolute -right-1.5 -top-1.5 grid h-5 w-5 place-items-center rounded-full bg-burgundy text-[9px] font-bold text-cream">{item.quantity}</span>
                  </div>
            ...
            <div className="mt-5 space-y-2 border-t border-border pt-4 text-sm">
              ...
              <div className="flex justify-between font-bold text-base border-t border-border pt-2"><span>Total</span><span className="text-burgundy">${total.toFixed(2)}</span></div>
            </div>
          </div>
        </div>
      </main>
