...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "Premium Designer Bags | Italy & Dubai" },
      { name: "description", content: "Curated luxury designer handbags from Italy & Dubai. LV, Chanel, Hermès, Dior, Celine and more. Worldwide shipping, secure payment, WhatsApp only ordering." },
      { property: "og:title", content: "Premium Designer Bags" },
      { property: "og:description", content: "Luxury redefined. Premium designer bags shipped from Italy & Dubai." },
    ],
  }),
  component: Index,
});
...
const SHOES: Product[] = [
  ...
  { title: "Bottega Veneta Stretch Flat Sandal — Cream Intrecciato", price: "$359", tag: "BV",         img: bag7 },
  ...
  { title: "Coach Lowline Low Top Sneaker — Signature Canvas",   price: "$189", tag: "Coach",          img: bag3 },
...
const COLLECTION: Product[] = [
  "LV Blue Collection Neverfull MM Tote M27358 — 47×28×14 cm|$369|LV",
  ...
  "The Row Margaux White Leather Set — L/M/S|$569|The Row",
  ...
  "Saint Laurent Cassandre Large Flap Wallet — Beige|$178|YSL",
  ...
  const [title, price, tag] = s.split("|");
  return { title, price, tag, img: pick(i) };
});
...
function Index() {
  return (
    <div className="min-h-screen bg-background text-foreground" style={{ fontFamily: "var(--font-sans)" }}>
      ...
      <ProductSection id="shoes" title="Designer Shoes" subtitle="Luxury footwear from the world's finest houses" products={SHOES} />
      <CategoriesGrid />
    ...
    <div className="text-xs text-cream" style={{ background: "var(--gradient-luxe)" }}>
      <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-2 px-4 py-2">
        <div className="flex items-center gap-1.5">
          ...
          <span>Italy & Dubai · UK · USA</span>
          <span className="hidden items-center gap-1.5 sm:flex ml-4"><Sparkles className="h-3 w-3 text-gold" /> Free worldwide shipping over $250</span>
        </div>
        <div className="flex items-center gap-4">
          <a href={INSTAGRAM_LINK} target="_blank" rel="noopener noreferrer" aria-label="Instagram" className="flex items-center gap-1.5 hover:text-gold">
            ...
            <span className="hidden sm:inline">@the_diamond_bags</span>
          </a>
          <a href={TIKTOK_LINK} target="_blank" rel="noopener noreferrer" aria-label="TikTok" className="flex items-center gap-1.5 hover:text-gold">
          ...
          <a href={WHATSAPP_LINK} className="flex items-center gap-1.5 hover:text-gold">
            <Phone className="h-3 w-3 text-gold" /> {WHATSAPP}
          </a>
        </div>
      </div>
    </div>
            ...
            <div key={c.name} className="group relative">
              ...
              <div className="invisible absolute left-1/2 top-full z-50 w-56 -translate-x-1/2 border-t-2 border-gold bg-card opacity-0 shadow-2xl transition group-hover:visible group-hover:opacity-100">
                <ul className="py-2">
                  {c.items.map((it) => (
                    <li key={it}>
                      <a href="#collection" className="block px-4 py-1.5 text-sm text-foreground/80 hover:bg-secondary hover:text-burgundy">{it}</a>
                    </li>
                  ))}
                </ul>
              </div>
          ...
          <div className="max-h-[70vh] overflow-y-auto px-4 py-4">
            {NAV_CATEGORIES.map((c) => (
              <details key={c.name} className="border-b border-border py-2">
                ...
                <ul className="pl-3">
                  {c.items.map((it) => (
                    <li key={it}><a href="#collection" className="block py-1 text-sm text-muted-foreground">{it}</a></li>
                  ))}
                </ul>
              </details>
            ))}
      ...
      <div className="mx-auto grid max-w-7xl gap-12 px-4 py-16 md:py-24 lg:grid-cols-[1.1fr_1fr] lg:items-center">
        <div>
          ...
          <p className="mt-7 max-w-md text-base leading-relaxed text-muted-foreground">
            A curated house of premium designer bags — Louis Vuitton, Chanel, Hermès, Dior and more — hand-shipped from Italy & Dubai with real-photo confirmation.
          </p>
...
function Marquee() {
  const items = ["Free Worldwide Shipping", "Master Quality", "WhatsApp Only Ordering", "Real Photos Before Dispatch", "Worldwide Delivery"];
  return (
...
function FeaturesStrip() {
  const items = [
    { icon: Truck, t: "Free Shipping", d: "Worldwide delivery" },
    { icon: ShieldCheck, t: "Master Quality", d: "Genuine craft & materials" },
    { icon: MessageCircle, t: "WhatsApp Only", d: WHATSAPP },
    { icon: RotateCcw, t: "Easy Returns", d: "Quality guarantee" },
  ];
  return (
  ...
  return (
    <a href={WHATSAPP_LINK} className="group block">
      <div className="relative aspect-square overflow-hidden bg-card">
        <img src={p.img} alt={p.title} width={1024} height={1024} loading="lazy" className="h-full w-full object-cover transition duration-700 group-hover:scale-105" />
        {p.tag && (
          <span className="absolute left-3 top-3 bg-cream/95 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-burgundy backdrop-blur">{p.tag}</span>
        )}
    ...
    <section className="relative border-b border-border" style={{ background: "linear-gradient(180deg, var(--secondary), var(--background))" }}>
      <div className="mx-auto max-w-7xl px-4 py-16 md:py-20">
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4 lg:grid-cols-8">
          {CATEGORIES.map((c) => (
            <a key={c.name} href="#collection" className="group flex aspect-square flex-col items-center justify-center border border-border bg-card text-center transition hover:-translate-y-1 hover:border-gold hover:shadow-lg" style={{ boxShadow: "0 2px 0 0 oklch(0.88 0.08 80 / 0.3)" }}>
    ...
    <section id="contact" className="border-b border-border bg-card">
      <div className="mx-auto max-w-7xl px-4 py-24 text-center">
        ...
        <h2 className="mx-auto max-w-3xl text-5xl tracking-tight text-ink md:text-6xl" style={{ fontFamily: "var(--font-display)" }}>
          Send us a message. <span className="italic text-burgundy">Get your bag.</span>
        </h2>
        ...
        <div className="grid gap-10 md:grid-cols-2 lg:grid-cols-4">
          <div>
            ...
            <p className="mt-4 text-sm leading-relaxed opacity-75">
              Premium designer bags — curated luxury shipped worldwide from Italy & Dubai.
            </p>
            <div className="mt-5 flex gap-3">
              <a href={INSTAGRAM_LINK} target="_blank" rel="noopener noreferrer" aria-label="Instagram" className="border border-gold/40 p-2 transition hover:bg-gold hover:text-burgundy-deep">
                <svg className="h-4 w-4" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/></svg>
              </a>
              <a href={TIKTOK_LINK} target="_blank" rel="noopener noreferrer" aria-label="TikTok" className="border border-gold/40 p-2 transition hover:bg-gold hover:text-burgundy-deep">
              ...
              <a href={WHATSAPP_LINK} target="_blank" rel="noopener noreferrer" aria-label="WhatsApp" className="border border-gold/40 p-2 transition hover:bg-gold hover:text-burgundy-deep"><MessageCircle className="h-4 w-4" /></a>
              <a href={`mailto:${EMAIL}`} aria-label="Email" className="border border-gold/40 p-2 transition hover:bg-gold hover:text-burgundy-deep"><Mail className="h-4 w-4" /></a>
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            <ul className="space-y-3 text-sm opacity-80">
              <li className="flex items-start gap-2"><MapPin className="mt-0.5 h-4 w-4 shrink-0 text-gold" /> Premium Designer Bags House — Italy & Dubai</li>
              <li className="flex items-start gap-2"><MessageCircle className="mt-0.5 h-4 w-4 shrink-0 text-gold" /> WhatsApp only: {WHATSAPP}</li>
